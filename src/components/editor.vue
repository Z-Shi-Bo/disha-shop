<template>
  ...
  <choose-image ref="chooseImageRef" :preview="false" :limit="9" />
</template>
<script setup>
...
const init = {
  ...
  toolbar:
    'formats undo redo fontsizeselect fontselect ltr rtl searchreplace media imagUpload | outdent indent aligncenter alignleft alignright alignjustify lineheight underline quicklink h2 h3 blockquote numlist bullist table removeformat forecolor backcolor bold italic strikethrough hr link preview fullscreen help ',
  ...
  setup: (editor) => {
    editor.ui.registry.addButton('imagUpload', {
      icon: 'image',
      tooltip: '插入图片',
      onAction: () => {
        chooseImageRef.value.beforeUpload((urlList) => {
          urlList.map((url) => {
